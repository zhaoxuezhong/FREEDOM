# 在其他模块中使用freedom-logging
###1、pom引入jar
```
<dependency>
    <groupId>com.zxz</groupId>
    <artifactId>freedom-logging</artifactId>
    <version>0.0.1-SNAPSHOT</version>
</dependency>
 ```
 ###2、启动类添加 scanBasePackages
 ```
 @SpringBootApplication(scanBasePackages = {"com.zxz.freedombase", "com.zxz.freedomlogging"})
 ```
 做完以上两步就已经为com.zxz.*.controller包下的类添加监控日志的功能了，但是还想为其他包下的类添加或自定义日志打印输出格式，请继续往下看
 ###3、@Log
```
@Log
public String test(){
...
```
给方法添加注解@Log就可以实现日志监控功能了
###4、配置Logging
* 在resources添加文件logback-spring.xml，内容参考：
```
...
logback:
  logDir: log
  appName: freedom-base

#ALWAYS 开启彩色输出
#DETECT 尝试检测ANSI 颜色是否支持
#NEVER 关闭彩色输出
spring:
  output:
    ansi:
